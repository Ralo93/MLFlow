...
rf_pipe = Pipeline(steps=[('preprocessor', preprocessor), ('lgbm', lgbm)])
...
def cross_validate_with_downsampling(params, X, y):
    ...
    rf_pipe.set_params(
    lgbm__n_estimators=params['n_estimators'],
    lgbm__learning_rate=params['learning_rate'],
    lgbm__max_depth=params['max_depth'],
    lgbm__subsample=params['subsample'],
    lgbm__lambda_l2=params['reg_lambda'],  # L2 regularization
    lgbm__colsample_bytree=params['colsample_bytree'],
    lgbm__min_child_weight=int(params['min_child_weight']),
    lgbm__lambda_l1=params['reg_alpha']  # L1 regularization
    )
    ...
    with mlflow.start_run(nested=True):
        ...
        mlflow.set_tag("tag", "autoencoders")
...
mlflow.set_experiment("/lgbm Earthquake final")
...
with mlflow.start_run():
    ...
    lightgbm = best_run["model"].named_steps['lgbm']
